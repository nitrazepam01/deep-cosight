"""Optional Co-Sight SkillFunction builders; imported only inside Co-Sight."""
...
def parse_control_task_skill():
    return _skill(
        "parse_control_task", "fbfb7a01-53f0-4b26-aefe-d009ac2a0001", "控制任务校验", "Validate Control Task",
        "校验结构化控制任务；不从自由文本猜测模型参数",
        "Validate a structured control task without guessing model parameters from free text",
        {"task_payload": TASK_PAYLOAD, "source_format": {"type": "string", "enum": ["auto", "json", "yaml"]}, "execution_id": {"type": "string"}},
        ["task_payload"],
    )
...
def analyze_plant_skill():
    return _skill(
        "analyze_plant", "fbfb7a01-53f0-4b26-aefe-d009ac2a0002", "控制对象分析", "Analyze Plant",
        "分析极点、零点、稳定性、适定性、可控性、可观性和条件数",
        "Analyze poles, zeros, stability, well-posedness, controllability, observability, and conditioning",
        {"task_payload": TASK_PAYLOAD, "execution_id": {"type": "string"}}, ["task_payload"],
    )


def design_controller_skill():
    return _skill(
        "design_controller", "fbfb7a01-53f0-4b26-aefe-d009ac2a0003", "控制器设计", "Design Controller",
...
def simulate_system_skill():
    return _skill(
        "simulate_system", "fbfb7a01-53f0-4b26-aefe-d009ac2a0004", "控制系统仿真", "Simulate Control System",
        ...
        {"task_payload": TASK_PAYLOAD, "controller_payload": {"type": "object"}, "execution_id": {"type": "string"}},
        ["task_payload"],
    )
...
def verify_requirements_skill():
    return _skill(
        "verify_requirements", "fbfb7a01-53f0-4b26-aefe-d009ac2a0005", "控制指标验证", "Verify Requirements",
        "将确定性指标与结构化性能要求逐项比较",
        "Compare deterministic metrics against structured performance requirements",
        {"metrics_payload": {"oneOf": [{"type": "object"}, {"type": "string"}]}, "requirements_payload": {"type": "array", "items": {"type": "object"}}, "execution_id": {"type": "string"}},
...
def run_control_workflow_skill():
    return _skill(
        "run_control_workflow", "fbfb7a01-53f0-4b26-aefe-d009ac2a0006", "控制设计完整流程", "Run Control Workflow",
...
CONTROL_SKILL_BUILDERS = {
    "parse_control_task": parse_control_task_skill,
    "analyze_plant": analyze_plant_skill,
    "design_controller": design_controller_skill,
    "simulate_system": simulate_system_skill,
    "verify_requirements": verify_requirements_skill,
    "run_control_workflow": run_control_workflow_skill,
}
